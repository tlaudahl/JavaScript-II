...
let fullNames = [];
runners.forEach(e => fullNames.push(`First: ${e.first_name} Last: ${e.last_name}`))
console.log(fullNames);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.
let firstNamesAllCaps = [];
runners.map(e => firstNamesAllCaps.push(e.first_name.toUpperCase()));
...
runners.forEach(e => {
  if (e.shirt_size === 'XS') {
    shirtSizes[0].XS += 1;
  } else if (e.shirt_size === 'S') {
    shirtSizes[0].S += 1;
  } else if (e.shirt_size === 'M') {
    shirtSizes[0].M += 1;
  } else if (e.shirt_size === 'L') {
    shirtSizes[0].L += 1;
  } else if (e.shirt_size === 'XL') {
    shirtSizes[0].XL += 1;
  } else if (e.shirt_size === '2XL') {
    shirtSizes[0]['2XL'] += 1;
  } else if (e.shirt_size === '3XL') {
    shirtSizes[0]['3XL'] += 1;
  }
})
console.table(shirtSizes);